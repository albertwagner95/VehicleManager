...
namespace VehicleManager.Tests.Services
{
    public class VehicleServiceTests
    {
        ...
        [Fact]
        public void ShouldReturnUserIdByVehicleId()
        {
            //arrange
            var vehicles = new List<Vehicle>()
            {
                new Vehicle() { Id = 1, ApplicationUserID = "user1" },
                new Vehicle() { Id = 2, ApplicationUserID = "user1" },
                new Vehicle() { Id = 3, ApplicationUserID = "user3" }
            };

            var repository = new Mock<IVehicleRepository>();
            repository.Setup(x => x.GetVehicles()).Returns(vehicles.AsQueryable());

            var mock = new Mock<IMapper>();
            var service = new VehicleService(repository.Object, mock.Object);

            var repositoryWithoutElements = new Mock<IVehicleRepository>();
            var serviceWithoutElements = new VehicleService(repositoryWithoutElements.Object, mock.Object);

            //act
            var firstUserId = service.GetUserIdByVehicleId(1);
            var secondUserId = service.GetUserIdByVehicleId(2);
            var thirdUserId = service.GetUserIdByVehicleId(3);
            var fourthUserId = service.GetUserIdByVehicleId(5);
            var idWithoutElements = serviceWithoutElements.GetUserIdByVehicleId(10);
            //asserts
            firstUserId.Should().NotBeNullOrEmpty();
            firstUserId.Should().Be("user1");

            secondUserId.Should().NotBeNullOrEmpty();
            secondUserId.Should().Be("user1");

            thirdUserId.Should().NotBeNullOrEmpty();
            thirdUserId.Should().Be("user3");

            fourthUserId.Should().BeNull();

            idWithoutElements.Should().BeNull();
        }
    }
}
